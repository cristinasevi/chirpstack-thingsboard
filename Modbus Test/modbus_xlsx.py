...
ws_rssi = wb.active
ws_rssi.title = 'RSSI'
ws_channels = wb.create_sheet(title='Channels')

# Escribir encabezados en ambas hojas
headers = ['Datetime'] + devices
ws_rssi.append(headers)
...
if not connection_successful:
    ...
    try:
        while True:
                ...
                if isinstance(device_config, dict) and 'addresses' in device_config and isinstance(device_config['addresses'], dict):
                    ...
                    for name, address in addresses.items():
                        ...
                        if value is not None:
                            ...
                            elif name.startswith("channel"):  # Procesar canales
            ...
            # Añadir las nuevas filas a las hojas de Excel
            ws_rssi.append(new_row_rssi)
            ws_channels.append(new_row_channels)
            print(f"Datos RSSI y Channels registrados para {timestamp}")

            # Guardar el archivo Excel
            wb.save('modbus_data.xlsx')
